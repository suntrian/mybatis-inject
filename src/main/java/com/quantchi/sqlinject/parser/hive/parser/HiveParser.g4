...
statements
    : ((explainStatement | execStatement) ';')*  (explainStatement | execStatement) ';'?  EOF
    ;
...
invalidateStatement
    : KW_INVALIDATE KW_METADATA tableName
    ;
...
insertClause
    ...
    | KW_INSERT (KW_INTO|KW_OVERWRITE) KW_TABLE? tableOrPartition (LPAREN columnNameList RPAREN)? partitionSpec?
    ;
...
limitClause
   : KW_LIMIT ((numberOrPlaceholder COMMA)? numberOrPlaceholder)
   | KW_LIMIT numberOrPlaceholder KW_OFFSET numberOrPlaceholder
   ;
