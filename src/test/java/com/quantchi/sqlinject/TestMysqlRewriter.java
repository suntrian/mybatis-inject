...
public class TestMysqlRewriter {
    ...
    public void tesDualFiltertInject() {
        ...
        System.out.println(sql1);
        Assert.assertEquals("SELECT a,b ,c FROM table1 T1 JOIN (select x, y, z from table2 t2 JOIN  table3 t1 ON t2.m = t1.n WHERE (a = 1 OR b = 1)  ) t2 on t1.b = t2.y WHERE ( a = 1 OR b = 1 OR t1.a = 5 OR t2.y = 5 ) AND  x  = ? AND y = ?"
        ...
        System.out.println(sql2);
        Assert.assertEquals("SELECT a,b ,c FROM table1 T1 JOIN (select x, y, z from table2 t2 JOIN  table3 t1 ON t2.m = t1.n  ) t2 on t1.b = t2.y JOIN table1 T3 ON t2.y = t3.c  WHERE  ( (T1.a = 1 AND T3.a = 1) OR (T1.b = 1 AND T3.b = 1) OR t1.a = 5 OR t2.y = 5 )  AND  x  = ? AND y = ?"
                        .replaceAll(" ", "").toUpperCase(),
                sql2.replaceAll(" ", "").toUpperCase());
