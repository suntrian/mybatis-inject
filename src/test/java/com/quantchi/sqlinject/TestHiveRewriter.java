...
public class TestHiveRewriter {
    ...
    public void testRewriteSelect() {
        ...
        String sql7 = "SELECT a,b ,c FROM table1 T1 JOIN (select x, y, z from table2 t2 JOIN  table3 t1 ON t2.m = t1.n  ) t2 on t1.b = t2.y WHERE (x  = ${abcd} AND y = ${abcd} )";
        sql7 = inject.injectFilter(sql7, "T1", "t1.a = 5" );
        System.out.println(sql7);
        Assert.assertEquals("SELECT a,b ,c FROM table1 T1 JOIN (select x, y, z from table2 t2 JOIN  table3 t1 ON t2.m = t1.n WHERE t1.a = 5 ) t2 on t1.b = t2.y WHERE t1.a = 5 AND ((x  = ${abcd} AND y = ${abcd})) ".replaceAll(" ", "").toUpperCase(),
                sql7.replaceAll(" ", "").toUpperCase());
    }
    ...
    public void tesDualFiltertInject() {
        ...
        String sql1 = "SELECT a,b ,c FROM table1 T1 JOIN (select x, y, z from table2 t2 JOIN  table3 t1 ON t2.m = t1.n  ) t2 on t1.b = t2.y WHERE x  = ${abcd} OR y = ${abcd} ";
        ...
        System.out.println(sql1);
        Assert.assertEquals("SELECT a,b ,c FROM table1 T1 JOIN (select x, y, z from table2 t2 JOIN  table3 t1 ON t2.m = t1.n WHERE (a = 1 OR b = 1)  ) t2 on t1.b = t2.y WHERE ( a = 1 OR b = 1 OR t1.a = 5 OR t2.y = 5 ) AND  (x  = ${abcd} OR y = ${abcd})"
        ...
        System.out.println(sql2);
        Assert.assertEquals("SELECT a,b ,c FROM table1 T1 JOIN (select x, y, z from table2 t2 JOIN  table3 t1 ON t2.m = t1.n  ) t2 on t1.b = t2.y JOIN table1 T3 ON t2.y = t3.c  WHERE  ( (T1.a = 1 AND T3.a = 1) OR (T1.b = 1 AND T3.b = 1) OR t1.a = 5 OR t2.y = 5 )  AND  (x  = ${abcd} OR y = ${abcd})"
                        .replaceAll(" ", "").toUpperCase(),
                sql2.replaceAll(" ", "").toUpperCase());
